...
public class JmsTransactionAwareDataSourceProxy extends TransactionAwareDataSourceProxy {
	...
	protected Connection getTransactionAwareConnectionProxy(DataSource targetDataSource) {
		...
		logger.debug(targetDataSource.toString()+"======Deal with the JMS Session:getTransactionAwareConnectionProxy======");
	...
	private class TransactionAwareInvocationHandler implements InvocationHandler {
		...
		public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
			...
			logger.debug(proxy.toString()+"======TransactionAwareInvocationHandler======"+method.getName());
			if (method.getName().equals("equals")) {
				...
				logger.debug(proxy.toString()+" Rolling back proxied connection.");
				if (DataSourceUtils.isConnectionTransactional(this.target, this.targetDataSource)) {
	...
	private void maybeRegisterSynchronization() {
		...
		TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronizationAdapter() {
			...
			public void beforeCommit(boolean readOnly) {
				...
				logger.debug("---Excute TransactionSynchronizationAdapter beforeCommit---");
