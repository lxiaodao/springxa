...
public class CustomerControlSynchronousMessageTriggerAndRollbackTests {
	
	private static final Log logger = LogFactory.getLog(CustomerControlSynchronousMessageTriggerAndRollbackTests.class);
	...
	public void checkPostConditions() {
		logger.debug("------checkPostConditions is begin------");
		List<String> list = getMessages();
		assertEquals(2, list.size());
		assertEquals(0, SimpleJdbcTestUtils.countRowsInTable(jdbcTemplate, "T_FOOS"));
		logger.debug("------checkPostConditions is end---");
	}
	
	
	@BeforeTransaction
	public void befortx() {
		logger.debug("------init clearData------");
		clearData();
		logger.debug("------init clearData end------");
	...
	public void testReceiveMessageUpdateDatabase() throws Exception {
		...
		for (String name : list) {
			logger.debug("---testReceiveMessageUpdateDatabase insert begin---"+id);
			jdbcTemplate.update("INSERT INTO T_FOOS (id,name,foo_date) values (?,?,?)", id++, name, new Date());
			logger.debug("---testReceiveMessageUpdateDatabase insert end---"+id);
		}
